<template>
  <div>
    ...
    <v-row>
      ...
      <v-col>
        이름 : {{ userData.username }} 성씨 본관 :{{ userData.ancestor }}
        <div>
          <button @click="searchAncestor(userData.ancestor)">조상과 관련된 문화재 보기</button>
        </div>
      </v-col>
      <v-col>
        {{ userData.grade }}
        <v-dialog v-model="gradeInfo" width="600px">
          <template v-slot:activator="{ on, attrs }">
            <v-btn v-bind="attrs" v-on="on">신분제알아보기</v-btn>
          </template>
          <v-card>
            <v-card-title>
